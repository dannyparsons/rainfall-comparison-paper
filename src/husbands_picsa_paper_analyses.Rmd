---
title: "Husbands, Barbados Comparisons"
...
- Only one station analysed but would be interesting to compare results from other stations in Barbados/nearby islands
- Station coordinates to be confirmed by CIMH
...
sf_zm <- ne_countries(scale = "medium", returnclass = "sf")
ggplot(sf_zm) + 
  geom_sf() +
  ...
  coord_sf(xlim = c(-59.7, -59.2), ylim = c(13, 13.5), expand = FALSE)
rm(sf_zm)
```
...
- All products underestimate totals, even ERA5, in contrast to Zambia where most were similar and ERA5 was overestimating
- Correlation is still high
- CHIRPS has a huge underestimation (30%) compared to ERA5 (14%) and IMERG (18%)
- All have a bit less variability than the station (60% - 80%)
...
ggsave(here("results", "picsa-paper", 
                       paste0("barbados_", "syear_", "total_rain", "_", "all", ".png")),
       width = 12, height = 6)
```
...
df %>% 
  select(metric, CHIRPS, ERA5, IMERG) %>%
  kable() %>%
...
- Very different across the products, CHIRPS: big underestimation, ERA5: big overestimation, IMERG: underestimation
- Positive but relatively lower correlation compared to totals and rain days for Zambia
- ERA5 has the biggest bias absolute value (as expected)
- CHIRPS less variable but other two have the same variability as station

```{r yearly_n_rain_plots}
# p_syear_n <- gof_pr %>%
#   mutate(p = purrr::map(data, yearly_plots, gof_col = "gof__n_rain0p85", 
#                         stat_pr = "n_rain0p85", stat_st = "n_rain0p85__station", 
#                         product_name = product),
#          paths = here("results", "barbados", 
#                       paste0("barbaods_", "syear_", "n_rain0p85", "_", product, ".png")))
# 
...
# stats_tables(gof_syear, "gof__n_rain0p85")
```
...
- Correlations are quite high for all products but graphs show that they all underestimate the most extreme totals (300mm+) mostly in Oct & Nov but also in others
- CHIRPS has the most underestimation, as in the yearly totals. It never gets close to the 300mm+ values in any month

- Possibly the products cannot detect the very heavy rains/hurricanes
- The bias in the drier months is much less, even as a percentage, showing reasonable performance outside the heavy rainy months

```{r monthly_plots_total_rain_chirps}
...
g <- ggplot(hus_month %>% filter(product == "CHIRPS"), 
  ...
  ggtitle("Gauge vs estimate Monthly total rainfall at Husbands") +
  facet_wrap(~ month_abb)
#g
ggsave(here("results", "picsa-paper", 
                       paste0("barbados_", "monthly_", "total_rain", "_", "chirps", ".png")),
       width = 12, height = 6)
...
#          paths = here("results", "barbados", 
#                       paste0("barbados_", "month_", "total_rain", "_", product, ".png")))
# 
# walk2(p_month_total$paths, p_month_total$p, ggsave, width = 12, height = 6)
# walk(p_month_total$p, print)
```

```{r all_months_tables_total_rain, results="asis"}
stats_tables(gof_month, "gof__total_rain")
```

```{r months_tables_total_rain, results="asis"}
...
```{r all_month_tables_n_rain0p85, results="asis"}
stats_tables(gof_month, "gof__n_rain0p85")
```

```{r months_tables_n_rain0p85, results="asis"}
