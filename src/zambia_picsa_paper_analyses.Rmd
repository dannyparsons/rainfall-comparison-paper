...
zm_long_st <- zm_long_st %>%
  mutate(season = ifelse(as.character(month) %in% 5:10, "Dry", as.character(month)),
...
gof_syear_adj <- by_syear_st_adj %>%
  ...
  mutate(n = purrr::map_int(data, 
         ...
         gof__mean_rain = purrr::map(data, dgof, "mean_rain", "mean_rain__station", 
                                    na.rm = TRUE),
         gof__n_rain_t = purrr::map(data, dgof, "n_rain_t", "n_rain__station", na.rm = TRUE),
         gof__mean_rain_t = purrr::map(data, dgof, "mean_rain_t", "mean_rain__station", 
                                    na.rm = TRUE)
         )
...
cor_by_year <- by_syear_st_adj %>%
  ...
  ungroup() %>%
  mutate(product = toupper(substr(product, 1, nchar(product) - 5)),
         product = factor(product, levels = c("CHIRPS", "ERA5", "TAMSAT", "IMERG", "RFE2")))

cor_by_year_stack <- cor_by_year %>% 
  pivot_longer(cols = c("cor_pr", "cor_pr_t"))

ggplot(cor_by_year_stack, aes(x = station, y = value, fill = name)) +
  geom_col(position = "dodge") +
...
for (p in seq_along(products)) {
  df <- zm_year_rain %>% dplyr::filter(product == products[[p]])
